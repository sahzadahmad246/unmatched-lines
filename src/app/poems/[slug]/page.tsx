...
async function fetchPoem(slug: string): Promise<Poem | null> {
  try {
    ...
    console.log("API Response for slug", slug, ":", JSON.stringify(data, null, 2));
...
export async function generateMetadata({
  ...
  const title = typeof poem?.title === "string" ? poem.title : poem?.title?.en || "Poem Not Found";
  const author = typeof poem?.author === "string" ? poem.author : poem?.author?.name || "Unknown Author";
  const description = poem?.content?.en
    ? `${poem.content.en.slice(0, 150)}... - A poem by ${author} on Unmatched Lines`
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: poem?.coverImage || "https://unmatched-lines.vercel.app/default-poem-image.jpg",
          width: 800,
          height: 400,
          alt: `${title} by ${author}`,
        },
    ...
    twitter: {
      ...
      images: [poem?.coverImage || "https://unmatched-lines.vercel.app/default-poem-image.jpg"],
    },
    robots: {
      index: true,
      follow: true,
