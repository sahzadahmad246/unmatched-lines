...
export default function ManagePoems() {
  ...
  const PoemCard = ({ poem }: { poem: Poem }) => (
    ...
    >
      <div className="flex justify-between items-start">
        <h3 className="font-medium text-sm">{poem.title?.en || "Untitled"}</h3>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" className="h-7 w-7 -mr-1.5 -mt-1">
  ...
  return (
    <motion.div
      ...
      className="w-full"
    >
      <div className="bg-background rounded-lg md:border md:shadow-sm">
        <div className="p-1 md:p-1">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
            <div className="flex items-center gap-2">
              <BookOpen className="h-5 w-5" />
              <span className="text-xl md:text-2xl font-semibold">Manage Poetry</span>
            </div>
          ...
          <div className="mt-2">
            {loading ? (
              <div className="py-6 text-center">Loading poems...</div>
              ...
              <>
                      ...
                      <AnimatePresence>
                        {filteredPoems.map((poem) => (
                          <TableRow key={poem._id}>
                            <TableCell className="font-medium py-2 md:py-4">{poem.title?.en || "Untitled"}</TableCell>
                            <TableCell className="py-2 md:py-4">{poem.author?.name || "Unknown"}</TableCell>
                            <TableCell className="py-2 md:py-4">{getCategoryBadge(poem.category)}</TableCell>
                            <TableCell className="py-2 md:py-4">{getStatusBadge(poem.status)}</TableCell>
                ...
                {/* Mobile Card View */}
                <div className="grid md:hidden gap-2 grid-cols-1">
                  <AnimatePresence>
                    {filteredPoems.map((poem) => (
