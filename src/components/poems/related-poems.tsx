      ...
      {relatedPoems.byCategory.length > 0 && (
        <div className="mb-4">
          <motion.h3
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-xs sm:text-sm font-medium mb-1.5 sm:mb-2 text-muted-foreground flex items-center"
          >
            ...
            {relatedPoems.byCategory.slice(0, 3).map((poem) => (
              <motion.div key={poem._id} variants={item} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Link href={`/poems/${language}/${getSlug(poem)}`}>
                  <Card className="hover:bg-primary/5 transition-colors border-muted/50 overflow-hidden group">
