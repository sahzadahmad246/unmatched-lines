...
export function Top50({ slug }: Top50Props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch top 50 poems
        const poemRes = await fetch(`/api/poem?authorSlug=${encodeURIComponent(slug)}&top=50`, {
          credentials: "include",
        });
        if (!poemRes.ok) throw new Error("Failed to fetch top 50 poems");
